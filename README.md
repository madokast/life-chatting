...
- **🌓 主题切换**：支持浅色/深色两套主题方案，默认跟随系统设置，支持热切换，系统模式变化时自动同步
...
#### 本地存储
- **本地优先原则**：所有用户数据默认优先存储在本地设备，确保数据安全性和隐私保护，无需依赖云端服务即可正常使用核心功能
- **Web 端实现**：采用 IndexedDB 存储所有数据，包括用户配置、日记内容、聊天记录等，提供可靠的本地数据管理
- **移动端实现**：
  - 文本数据：使用本地数据库（IndexedDB 或 SQLite）存储
  - 多媒体文件：通过原生文件系统 API 访问本地存储，避免浏览器 API 限制，提供更大的存储容量和更可靠的存储方式
