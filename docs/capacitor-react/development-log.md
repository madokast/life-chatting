...
### Posts 组件更新
- 添加了 posts 状态管理
- 实现了 handleAddPost 回调函数
- 新增日记自动添加到列表顶部
- 集成了 PostInput 组件

### 视图切换优化
- 修改了 App.tsx 中的视图切换逻辑
- 从条件渲染改为使用 display: none
- 避免组件在切换时卸载和重新加载
- 保持组件内部状态（如 AI 聊天回复进度）

### 国际化支持
- 更新了 i18n 翻译文件，添加了 post 相关翻译：
  - post.placeholder（输入框占位符）
  - post.add（添加按钮文本）
- 支持中英文切换

### 代码提交
- feat: 添加日记输入功能
- refactor: 修改视图切换方式，使用 display: none 保持组件状态
...
### 聊天组件开发
- 创建了 chat 子目录，与 diary 和 navigation 同级
- 创建了 UserMessageBubble 组件：
  - 用户聊天气泡，带背景颜色（light: #e3f2fd, dark: #1565c0）
  - 靠右对齐，宽度 80%
  - 圆角设计
- 创建了 AIMessageBubble 组件：
  - AI 聊天气泡，不带背景颜色
  - 靠左对齐，宽度 100%
- 创建了 ChatInput 组件：
  - 多行文本框（textarea）
...
### 视图切换功能
- 更新了 App.tsx，添加了 View 类型（'diary' | 'chat'）
- 实现了底部导航栏点击切换功能
- 移除了底部导航栏的"设置"项（已在 Drawer 中）
- 根据当前视图渲染 Posts 或 Chat

### 国际化支持
- 更新了 i18n 翻译文件，添加了聊天相关翻译：
  - chat.placeholder（输入框占位符）
  - chat.send（发送按钮文本）
- 支持中英文切换
...
- ✅ 聊天窗口功能开发完成
- ✅ 用户和 AI 消息气泡样式区分明显
- ✅ 视图切换功能实现，可在日记和聊天之间切换
- ✅ 所有组件支持 light/dark 主题切换
- ✅ 国际化支持完善
